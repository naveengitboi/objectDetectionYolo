...
def initialization_of_motor_two():
    ...
    return cap, model, motor_controller, speed_controller
...
def main():
    ...
    try:
        while True:
            ...
            print("Steps status ", step_status)

            should_update_speed = speed_controller.update_speed_of_motor_two(step_status)
            ...
            if should_update_speed:
                ...
                # send_to_dashboard(current_speed, len(step_status), 0, 'pickup_belt')
            show_metrics_two(frame,area_till_now, current_speed)
            # Visualize ROIs on the frame
            for idx, roi in enumerate(rois):
                top, bottom, left, right = roi
